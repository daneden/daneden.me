...
- With a leading `+44`, the international dialling code for UK numbers, followed
  by 10 digits.
...
[^1]:
  There are also _non-capturing groups_, which have a slightly different syntax:
  they open with `(?:` instead of just `(`. Capture groups have a simpler
  syntax, and allow us to reference the matching value later on.
...
[^2]:
  ...
  reference the first capture group, whereas in PHP or Python, you’d write `\1`.
...
[^4]:
  For example, Visa cards always start with a 4; MasterCard numbers always start
  with a 5 followed by a number between 1–5; Amex always starts with 34 or 37.
  [This post](https://www.regular-expressions.info/creditcard.html) goes into
  regex patterns for credit card numbers in some depth, but be advised that the
  patterns therein are quite complex (though they use only the features of regex
  we’re covering here!).
