  ...
  class ImageTag < Liquid::Tag
    ...
    def render(context)
      if @class && (@class.match('alignleft') or @class.match('alignright'))
        ...
        if @local
          source += "<picture>"
          source += "<source srcset=\"#{@name}-small.#{@ext}\" media=\"(max-width: 320px)\" />"
          source += "<source srcset=\"#{@name}-small@2x.#{@ext}\" media=\"(max-width: 320px), (-webkit-min-device-pixel-ratio: 1.5), (min-resolution: 144)\" />"
          source += "<source srcset=\"#{@name}-medium.#{@ext}\" media=\"(max-width: 640px)\" />"
          source += "<source srcset=\"#{@name}-medium@2x.#{@ext}\" media=\"(max-width: 640px), (-webkit-min-device-pixel-ratio: 1.5), (min-resolution: 144)\" />"
          source += "<source srcset=\"#{@name}-large.#{@ext}\" media=\"(max-width: 1024px)\" />"
          source += "<source srcset=\"#{@name}-large@2x.#{@ext}\" media=\"(max-width: 1024px), (-webkit-min-device-pixel-ratio: 1.5), (min-resolution: 144)\" />"
