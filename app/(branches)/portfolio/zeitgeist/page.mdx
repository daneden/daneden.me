...
export const metadata = {
    ...
    description:
        "A deep dive into how the Zeitgeist app for iOS and macOS was designed and built",
    openGraph: {
        title: "Deep Dive: Zeitgeist",
        description:
            "A deep dive into how the Zeitgeist app for iOS and macOS was designed and built",
...
app, and are more powerful than they first appear. The first step in getting
notifications set up is registering a device with Apple's Push Notification
Service (APNS). This happens across several steps:
...
3. This token is then sent to Zeitgeist's “Postal Service” (ZPS) server, one of
   ...
   token, the current user's Vercel account ID is sent to the ZPS server.
...
1. When the user manually refreshes the page
2. When the app reenters the foreground from the background
3. When the view first appears
...
To manage SWR-style caching, Swift’s Foundation framework provides URLCache,
